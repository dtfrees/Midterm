...
g.
There is a large ($\eta^2 = 0.47$, CI [0.34,1.00]) difference between soil phosphorus levels between areas with different times since burn (Single-Factor ANOVA, F(2,102) = 4.1822, p-value = 0.018, $\alpha = 0.05$).  Unburned areas had the least amount of mean soil phosphorus levels ($\bar{x} \approx ~0.80 ppm$, CI [?,?]), and recent burn areas had the highest mean soil phosphorus levels ($\bar{x} \approx 1.14 ppm$, CI [?,?]).


Problem 2

a. They did the Crab Feeding Trials to test whether crabs consume below-ground roots when presented algae at the same time. They ran Welch’s two-sample t-test to accounts for the different variance structures between the three trials.

b. They had one trial with just pickleweed roots, one with just algae, and one with pickleweed and algae but half the amount of each in the previous two trials. Each trial control (no-crabs) and a treatment (crabs) and ran for 72 hours.

c. The differecne in mean dry weights was that if there was a higher mean dry weight that meant that the crabs ate less of the algae or root in that trial.  In every trial, the control had a higher mean dry weight than the treatment and this is because in the control there were no crabs introduced to consume the algae or roots, while there were crabs introduced in the treatment.
...
CFTdf_control <- CFT_Data %>% #Create a df for the control data for trial 1 and 2
  select(Assigned_TTT_old, Assigned_TTT, Type, Planned_Starting_FW_g, Post_Trial_Dryweight_g) %>% #get desired columns
  group_by(Assigned_TTT) %>% #group by trial #
  filter(Type == "Control" & Assigned_TTT %in% c("trial (ii)", "trial (i)")) #filter the dataframe for only "Control" rows and then only by trial (ii) & (i) within the control rows

CFTdf_treatment <- CFT_Data %>% #Create a df for the treatment data for trial 1 and 2
  select(Assigned_TTT_old, Assigned_TTT, Type, Planned_Starting_FW_g, Post_Trial_Dryweight_g) %>% #get desired columns
  group_by(Assigned_TTT) %>% #group by trial #
  filter(Type == "Treatment" & Assigned_TTT %in% c("trial (ii)", "trial (i)")) #filter the dataframe for only "Treatment" rows and then only by trial (ii) & (i) within the Treatment rows

```

```  {r T3}
CFTdf_cnALT3 <- CFT_Data %>% #Create a df for the algae control for trial (iii)
  select(Assigned_TTT_old, Assigned_TTT, Type, Planned_Starting_FW_g, Post_Trial_Dryweight_g, Tissue) %>%  #get desired columns
  group_by(Assigned_TTT) %>% #group by trial #
  filter(Type == "Control" & Assigned_TTT %in% c("trial (iii)") & Tissue == "Algae") #filter the dataframe for only Control rows that are also trial (iii) and the Tissue is Algae

CFTdf_cnPWT3 <- CFT_Data %>% #Create a df for the Pickleweed control for trial (iii)
  select(Assigned_TTT_old, Assigned_TTT, Type, Planned_Starting_FW_g, Post_Trial_Dryweight_g, Tissue) %>% #get desired columns
  group_by(Assigned_TTT) %>% #group by trial #
  filter(Type == "Control" & Assigned_TTT %in% c("trial (iii)") & Tissue == "Pickleweed Roots") #filter the dataframe for only Control rows that are also trial (iii) and the Tissue is Pickleweed

CFTdf_trALT3 <- CFT_Data %>% #Create a df for the algae treatment for trial (iii)
  select(Assigned_TTT_old, Assigned_TTT, Type, Planned_Starting_FW_g, Post_Trial_Dryweight_g, Tissue) %>% #get desired columns
  group_by(Assigned_TTT) %>% #group by trial #
  filter(Type == "Treatment" & Assigned_TTT %in% c("trial (iii)") & Tissue == "Algae") #filter the dataframe for only Treatment rows that are also trial (iii) and the Tissue is Algae

CFTdf_trPWT3 <- CFT_Data %>% #Create a df for the pickleweed treatment for trial (iii)
  select(Assigned_TTT_old, Assigned_TTT, Type, Planned_Starting_FW_g, Post_Trial_Dryweight_g, Tissue) %>% #get desired columns
  group_by(Assigned_TTT) %>% #group by trial #
  filter(Type == "Treatment" & Assigned_TTT %in% c("trial (iii)") & Tissue == "Pickleweed Roots") #filter the dataframe for only Treatment rows that are also trial (iii) and the Tissue is Pickleweed

```

e.
```{r}
#Control Means and SD T1 & T2
mean_sd_ctr <- summarise(CFTdf_control, meanCFT = # make a dataframe with the mean and sd of the control experiments from trials (i) and (ii)
                   mean(Post_Trial_Dryweight_g), 
                   sdCFT =
                   sd(Post_Trial_Dryweight_g))

#Treatment Means and SD T1 & T2
mean_sd_trmt <- summarise(CFTdf_treatment, meanCFT = # make a dataframe with the mean and sd of the Treatment experiments from trials (i) and (ii)
                   mean(Post_Trial_Dryweight_g),
                   sdCFT =
                   sd(Post_Trial_Dryweight_g))

#Control Mean and SD PW T3
T3PW_mean_sd_ctr <- summarise(CFTdf_cnPWT3, meanCFT = # make a dataframe with the mean and sd of the pickleweed control experiments from trial (iii)
                   mean(Post_Trial_Dryweight_g),
                   sdCFT =
                   sd(Post_Trial_Dryweight_g)) %>% 
  mutate(Assigned_TTT = case_when(Assigned_TTT == 
                                    "trial (iii)" ~ "trial (iii) pickleweed")) #change the name of the trial column so I can differentiate between the different experiments within trial (iii)

#Treatment Means and PW SD T3
T3PW_mean_sd_trmt <- summarise(CFTdf_trPWT3, meanCFT = # make a dataframe with the mean and sd of the pickleweed Treatment experiments from trial (iii)
                   mean(Post_Trial_Dryweight_g),
                   sdCFT =
                   sd(Post_Trial_Dryweight_g)) %>% 
  mutate(Assigned_TTT = case_when(Assigned_TTT == 
...
#Control Mean and SD AL T3
T3AL_mean_sd_ctr <- summarise(CFTdf_cnALT3, meanCFT = # make a dataframe with the mean and sd of the Algae control experiments from trial (iii)
                   mean(Post_Trial_Dryweight_g),
                   sdCFT =
                   sd(Post_Trial_Dryweight_g)) %>% 
  mutate(Assigned_TTT = case_when(Assigned_TTT == 
                                    "trial (iii)" ~ "trial (iii) algae")) #change the name of the trial column so I can differentiate between the different experiments within trial (iii)

#Treatment Means and SD AL T3
T3AL_mean_sd_trmt <- summarise(CFTdf_trALT3, meanCFT = # make a dataframe with the mean and sd of the Algae Treatment experiments from trial (iii)
                   mean(Post_Trial_Dryweight_g),
                   sdCFT =
                   sd(Post_Trial_Dryweight_g)) %>% 
  mutate(Assigned_TTT = case_when(Assigned_TTT == 
...
mean_sd_T1_2 <- merge(mean_sd_ctr, #merge the control and treatment mean and sd tables so I have one summary table
                 mean_sd_trmt,
                 by = "Assigned_TTT") %>% 
  rename("Control Mean" = "meanCFT.x") %>% #rename the columns to be able to differentiate Control from treatment and mean from SD. This also make merging the table and matching the columns smoother.
  rename("Control SD" = "sdCFT.x") %>% 
  rename("Treatment Mean" = "meanCFT.y") %>% 
  rename("Treatment SD" = "sdCFT.y")
  
mean_sd_ALT3 <- merge(T3AL_mean_sd_ctr, T3AL_mean_sd_trmt,#merge the control and treatment mean and sd tables
                    by = "Assigned_TTT")  %>% 
  rename("Control Mean" = "meanCFT.x") %>% #rename the columns to be able to differentiate Control from treatment and mean from SD. This also make merging the table and matching the columns smoother
  rename("Control SD" = "sdCFT.x") %>% 
  rename("Treatment Mean" = "meanCFT.y") %>% 
...
mean_sd_PWT3 <- merge(T3PW_mean_sd_ctr, T3PW_mean_sd_trmt,#merge the control and treatment mean and sd tables
                    by = "Assigned_TTT") %>% 
  rename("Control Mean" = "meanCFT.x") %>% #rename the columns to be able to differentiate Control from treatment and mean from SD. This also make merging the table and matching the columns smoother
  rename("Control SD" = "sdCFT.x") %>% 
  rename("Treatment Mean" = "meanCFT.y") %>% 
...
mean_sd <- bind_rows(mean_sd_T1_2, mean_sd_ALT3, mean_sd_PWT3) # bind the final data frame together to have all trial means and standard deviations

print(mean_sd) #show the output df
```
...
ggplot(data = CFT_Data, #use original dataframe
              aes(x = Post_Trial_Dryweight_g, #x-axis is the dryweight of algae or pickleweed left
              y = Type, # y is the type of experiment it was control or treatment (normally on the x but for a horizontal jitter this is required)
              color = Tissue)) + # classify the colors of th data according
  geom_jitter(height = 0.1, # change the "width" of the horizontal jitter
   ...
   stat_summary(geom = "pointrange", # make the 
               ...
               size = 0.6) + #make the mean point larger to differentiate from the underlying data
  ...
  labs(x = "Post-Trial Mean Dryweight (g) # add an x-axis title
       +/- Std. Error",
       y = "")  # take out the y-axis title because we only need the tick marks to explain it
```

h.
Fig. 1: Crab Feeding Trial Results:
Showing mean amounts (Highlighted dots) of Dryweight Pickleweed (S. Pacifica) Root or Algae (Ulva Lactuca) left after being a jar for 72 hours Without Crab (Control) vs. With Crab (Treatment) ($\pm$ SEM (whiskers)). The transparent points underneath the Mean and SE are the underlying data points from observation within the trial. Green represents the Algae and blue represents Pickleweed roots. In trial (iii) both Pickleweed roots and algae were put in the jar. 
Data citation: Beheshti, Kathryn (2022). Long-term study reveals top-down effect of crabs on a California salt marsh [Dataset]. Dryad. https://doi.org/10.7291/D1H971 
...
Cold_Plunge_WTbW_Mean <- Cold_Plunge_Clean %>% # create a dataframe with the means by different weather type
  ...
  rename(`Mean Water Time` = mean)  # rename the column
...
ggplot(Cold_Plunge_Clean, # plot the initial clean df
  ...
  geom_point(data = Cold_Plunge_WTbW_Mean, # use the mean sumary table i created
             aes(x = Weather, #nweather type on the x
                 y = `Mean Water Time`, # water time on the y
...
the 95% confidence interval for each weather type. The data points under-
...
ft <- flextable(Cold_Plunge_WTbW_Mean)
ft <-set_caption(ft, caption = "Mean Seconds in Water by Weather Type") # caption not outputting in final render
ft
```
